...
const findUserById = async (userId) => {
  ...
  if (mongoose.Types.ObjectId.isValid(userId)) {
    user = await ExpertInformation.findById(userId);
  }

  // If not found or invalid ObjectId, try to find by custom ID field
  if (!user) {
    user = await ExpertInformation.findOne({
      $or: [
        { _id: userId },
        { id: userId },
        { userId: userId },
...
router.get("/debug/users", async (req, res) => {
  try {
    const users = await ExpertInformation.find({}, { _id: 1, id: 1, userId: 1, customId: 1, name: 1, email: 1 }).limit(10);
    res.json({
...
router.post("/debug/create-test-user", async (req, res) => {
  try {
    ...
    const existingUser = await ExpertInformation.findById("68c94094d011cdb0e5fa2caa");
    if (existingUser) {
      return res.json({
    ...
    const testUser = new ExpertInformation({
