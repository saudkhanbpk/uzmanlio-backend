...
export const createPackage = async (req, res) => {
    try {
        ...
        // Handle selectedClients - create orders for them
        const createdOrders = [];
        if (selectedClients && selectedClients.length > 0) {
            console.log(
                `Creating orders for ${selectedClients.length} selected clients`
            );
            ...
            for (const client of selectedClients) {
                try {
                            ...
                            events: [
                                {
                                    ...
                                    package: {
                                        ...
                                        sessions:
                                            newPackage.appointmentCount ||
                                            newPackage.sessionsIncluded ||
                                            1,
                    ...
                    console.error(
                        `Error creating order for client ${client.id || client}:`,
                        orderError
                    );
                }
            }
...
export const updatePackage = async (req, res) => {
    try {
        ...
        if (pkg.expertId.toString() !== user._id.toString()) {
            return res
                .status(403)
                .json({ error: "Not authorized to update this package" });
        }
        ...
        Object.assign(pkg, {
            ...
            price:
                req.body.price !== undefined ? parseFloat(req.body.price) : pkg.price,
            ...
            duration:
                req.body.duration !== undefined
                    ...
                    : pkg.duration,
            appointmentCount:
                req.body.appointmentCount !== undefined
                    ? parseInt(req.body.appointmentCount)
                    : pkg.appointmentCount,
            sessionsIncluded:
                req.body.appointmentCount !== undefined
                    ? parseInt(req.body.appointmentCount)
...
/**
 * Delete a package
 * DELETE /:userId/packages/:packageId
 */
export const deletePackage = async (req, res) => {
    try {
        ...
        const deletedPackageId = pkg._id;
 ...
 * PATCH /:userId/packages/:packageId/toggle-available
 */
export const togglePackageAvailable = async (req, res) => {
    try {
 ...
 * GET /:userId/packages/:packageId/purchases
 */
export const getPackagePurchases = async (req, res) => {
    try {
        const user = await findUserById(req.params.userId);
